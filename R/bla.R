...
# Representation Conventions and Shapes
# (Bias is absorbed into weight matrices as the first column)

# Input:
# - X: Input matrix of shape (batch_size × input_size), each row is one sample.

# Hidden layer:
# - W1: Weights of shape (hidden_size × (input_size + 1)), first column is bias
...
# Output layer:
# - W2: Weights of shape (output_size × (hidden_size + 1)), first column is bias
...
# - Each column corresponds to one neuron
# - Bias is folded into weights as the first column; no separate bias vectors are used.
#   This convention prepends a -1 to the input/hidden activations, making the first
...
train_network_with_absorbed_bias_gd <- function(
	...
	for (epoch in 1:epochs) {
		...
		for (start_idx in seq(1, num_samples, by = batch_size)) {
			...
			A1_bias <- cbind(-1, fwd$A1)
			X_bias <- cbind(-1, X_batch)

			# gradients
			grad_W2 <- t(error) %*% A1_bias / batch_size_actual

			grad_hidden <- error %*% network$W2[, -ncol(network$W2), drop = FALSE]  # hidden only
			grad_hidden_pre <- grad_hidden * (1 - fwd$A1^2)  # derivative of tanh
...
plot_results <- function(network, data, polynomial_fn, x_limits) {
	...
	x_plot <- seq(
		from = x_limits[1] - 0.1 * x_span,
		to   = x_limits[2] + 0.1 * x_span,
		length.out = 200
	)
...
save_training_frame <- function(
	epoch, network, frame_dir, frame_width, frame_height, true_fun, x_plot,
	xlim, ylim
) {
	frame_file <- file.path(frame_dir, sprintf("frame_%04d.png", epoch))
	png(frame_file, width = frame_width, height = frame_height)
	y_pred <- forward_pass(network, matrix(x_plot, ncol = 1))$output
...
train_network_bla <- function(
	...
	if (!is.null(extra)) {
		save_training_frame(
			0, network,frame_dir = extra$frame_dir,
			frame_width = extra$frame_width,
			frame_height = extra$frame_height, true_fun = extra$true_fun,
			x_plot = extra$x_plot, xlim = extra$xlim, ylim = extra$ylim
		)
	}
	...
	for (epoch in 1:num_epochs) {
		...
		if (!is.null(extra)) {
			save_training_frame(
				epoch, network,frame_dir = extra$frame_dir,
				frame_width = extra$frame_width,
				frame_height = extra$frame_height, true_fun = extra$true_fun,
				x_plot = extra$x_plot, xlim = extra$xlim, ylim = extra$ylim
			)
		...
		if (TRUE || epoch %% 10 == 0) {
			...
			# break out of mse is below a threshold for early stopping
			#if (mse < 1.0) {
			#	break
			#}
		}
...
main <- function(
	...
	x_plot <- seq(x_limits[1], x_limits[2], length.out = 500)
	y_true <- polynomial(x_plot)
	y_pad <- 0.1 * (max(y_true) - min(y_true))
	y_limits <- c(min(y_true) - y_pad, max(y_true) + y_pad)
	extra <- list(
